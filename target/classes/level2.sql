...
(
    ...
    departure_airport_code char(16) references airport (code) not null,
    arrival_date timestamp not null,
    arrival_airport_code char(16) references airport (code) not null,
    aircraft_id int references aircraft (id),
...
--insertions-------------------------------------//---------------------------------
-- первыми должны отрабатывать вставки таблиц, не имеющих FK
...
-- что происходит: в качестве значений в таблицу используется перекрестная вставка айдишников из таблицы
-- aircraft и захардкоженных значений seat_no
--т.е. т.о. заполняются места во всех самолетах (они одинаковы по самолетам)
...
where seat_no = 'B1'
  ...
  and f.departure_date::date = (now() - interval ' 3 years 8 month 10 days')::date;

-- 2. Сколько мест осталось свободными 2020-06-14 на рейсе MN3002?
-- идея состоит в том, чтобы из всех мест данного рейса вычесть количество проданных билетов
